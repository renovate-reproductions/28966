...
func TestApplyDiff(t *testing.T) {
	...
	m := make(ResourceMap)
	q := m["dummy"]
	q.Enqueue(NewDummy(10, 10))
	m["dummy"] = q

	diff := NewResourceDiff()
	diff.New["dummy"] = []Resource{NewDummy(0, 0)}
	m.ApplyDiff(diff)
	if len(m["dummy"]) != 2 || m["dummy"][1].Uid() != 0 {
	...
	m.ApplyDiff(diff)
	if len(m["dummy"]) != 2 || m["dummy"][1].Oid() != 1 {
	...
	m.ApplyDiff(diff)
	if len(m["dummy"]) != 1 || m["dummy"][0].Oid() != 10 {
		t.Errorf("failed to remove resource from diff")
	}
}

func TestApplyFullUpdateDiff(t *testing.T) {
